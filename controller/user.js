...
async function Usersignup(req, res) {
  ...
  if (email == "" || password == "" || role == "") {
    var response = {
      status: 201,
      message: "email, role and password can not be empty !!",
    };
    return res.status(201).send(response);
  }
                    ...
                    if (req.body.subcategory) {
                      ...
                      //console.log(typeof req.body.subcategory);
                        ...
                        for (var b = 0; b < req.body.subcategory.length; b++) {
                          //console.log(typeof req.body.subcategory[b]);
                          //console.log(docs.subcategory[b]);
                          docs.subcategory[b] = ObjectId(
                            req.body.subcategory[b]
                          );
                          //docs.subcategory.push(req.body.subcategory[b])
                          await docs.save();
                        }
...
async function BusinessUser(req, res) {
    ...
    if (users) {
      ...
      if (isfollow) {
        ...
        const data = await Notification.aggregate([
          {
            $match: {
              $and: [{ owner: user._id, user: users._id }, { type: "follow" }],
            },
          },
        ]);
